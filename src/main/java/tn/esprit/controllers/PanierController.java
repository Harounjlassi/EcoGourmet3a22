...
public class PanierController {
    ...
    @FXML
    private TableColumn<Annonce, Float> prixColumn;
    @FXML
    ...
    public void initialize() {
        ...
        prixColumn.setCellValueFactory(cellData -> {
            return new SimpleFloatProperty(cellData.getValue().getPrix()).asObject();
        });
    ...
    private void filtrerAnnonces(String searchText) {
        ...
        if (searchText.isEmpty()) {
            annoncesFiltrees.setAll(panierService.getAllAnnoncesFromPanier(loginController.id)); // Remplacez observableListToutesLesAnnonces avec votre liste complète d'annonces
        } else {
            ...
            for (Annonce annonce : annonces) {
                ...
                if (annonce.getNom_du_plat().toLowerCase().contains(searchText.toLowerCase())) {
...
//    }
    ...
    private void trierAnnoncesParPrixCroissant(ObservableList<Annonce> annonces) {
        annonces.sort(Comparator.comparingDouble(tn.esprit.models.Annonce::getPrix));
    }

    private void trierAnnoncesParPrixDecroissant(ObservableList<Annonce> annonces) {
        annonces.sort(Comparator.comparingDouble(tn.esprit.models.Annonce::getPrix).reversed());
    }

    private void trierAnnoncesParNomAZ(ObservableList<Annonce> annonces) {
        annonces.sort(Comparator.comparing(Annonce::getNom_du_plat));
    }

    private void trierAnnoncesParNomZA(ObservableList<Annonce> annonces) {
        annonces.sort(Comparator.comparing(Annonce::getNom_du_plat).reversed());
    }
